...
def test_parse_gmail_message_prefers_text_plain_body():
    raw_message = {
        "id": "msg-1",
        "threadId": "thread-1",
        "labelIds": ["INBOX"],
    ...
    assert parsed["body"] == "Hello text body"


def test_parse_gmail_thread_sorts_messages_by_internal_date():
    raw_thread = {
        "id": "thread-1",
        "messages": [
            {
    ...
    assert parsed["thread_id"] == "thread-1"
    assert [m["message_id"] for m in parsed["messages"]] == ["msg-1", "msg-2"]
